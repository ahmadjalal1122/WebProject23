    ...
    function TableRow({ pictures, _id, name, price }) {
        return (
            <tr>
                ...
                <td>{price}</td>
                <td>
                    <Button onClick={() => handleDeleteProduct(_id, user._id)} disabled={isLoading} style={{backgroundColor:"red",border:"0",margin:"4px"}}>
                        Delete
                    </Button>
                    <Link to={`/product/${_id}/edit`} className="btn btn-warning" style={{backgroundColor:"green",color:"white",border:"0",marginLeft:"2px"}}>
                        Edit
                    </Link>
                </td>
            </tr>
